  ...
  return (
    <PageContainer>
      <div className="pb-8">
          ...
          {scrolled ? (
            <div className="flex h-11 items-center justify-between">
              ...
              <button
                ...
                className="bg-sage text-on-sage text-13 flex h-8 cursor-pointer items-center gap-1 rounded-xs border-none px-3.5 font-semibold"
              >
            ...
            <div className="flex items-center justify-between pt-5 pb-3.5">
              ...
              <button
                onClick={() => setShowAdd(true)}
                className="bg-sage text-on-sage text-14 flex h-9 cursor-pointer items-center gap-1 rounded-xs border-none px-4 font-semibold"
              >
              ...
              >
                <div
                  ...
                  style={{
                    ...
                    outline: previewId === group.id ? '0.125rem solid var(--sage)' : undefined,
                    outlineOffset: previewId === group.id ? '-0.125rem' : undefined,
                  }}
                >
                  <div className="mb-2 flex items-center justify-between">
                    <div className="flex items-center gap-1.5">
                      <h3 className="text-15 text-text-primary font-semibold">{group.name}</h3>
                    </div>
                    {iAmInvolved && (
                      <span className="text-11 text-sage font-medium">You&apos;re a leader</span>
                    )}
                  </div>
                  ...
                  >
                    <span className="text-11 rounded-pill bg-sage-light text-sage px-2 py-0.5 font-medium">
                      {members.length} {members.length === 1 ? 'member' : 'members'}
                    </span>
                    <span className="text-11 rounded-pill bg-blue-light text-blue inline-flex items-center gap-1 px-2 py-0.5 font-medium">
                      <Crown size={11} />
                  ...
                  {group.description && (
                    <p
                      className="text-12 text-text-secondary mb-2.5 overflow-hidden leading-normal"
                      style={{
                        display: '-webkit-box',
                        WebkitLineClamp: 2,
        ...
        {showAdd && (
          <div
            className="z-modal fixed inset-0 flex items-center justify-center px-4"
            style={{
          ...
          >
            <div
              className="bg-surface w-full rounded-lg shadow-[var(--shadow-elevated)]"
              style={{ padding: '1.25rem 1.25rem 1.5rem', maxWidth: 480 }}
            >
              <div className="mb-5 flex items-center justify-between">
                <h2 className="text-17 text-text-primary font-bold">New Fellowship Group</h2>
                <button
                  onClick={() => setShowAdd(false)}
                  className="text-22 text-text-muted cursor-pointer border-none bg-transparent leading-none"
                >
                  ×
                </button>
              </div>
              ...
              <div className="flex flex-col gap-3">
                <div>
                  ...
                  <input
                    ...
                    className="border-border text-15 bg-bg text-text-primary box-border w-full rounded-sm border outline-none"
                ...
                <div>
                  ...
                  <textarea
                    ...
                    rows={3}
                    className="border-border text-14 bg-bg text-text-primary box-border w-full resize-none rounded-sm border outline-none"
                    style={{ padding: '0.625rem 0.75rem' }}
                  />
                </div>
              </div>
      ...
      {previewId && <GroupSidePreview groupId={previewId} onClose={() => setPreviewId(null)} />}
    </PageContainer>
  );
}

...
import { CaretRight, EnvelopeSimple, Lock, Question, SignOut, HandHeart, ShieldStar, Users, MapPin, Check, GoogleLogo } from '@phosphor-icons/react';
  ...
  return (
    <div style={{ paddingBottom: 48 }}>
      ...
      <SettingsCard>
        ...
        {hasPassword && !hasGoogle && (
          <button
            ...
            style={{ width: '100%', background: 'none', border: 'none', padding: 0, cursor: linkingGoogle ? 'not-allowed' : 'pointer', textAlign: 'left', display: 'block', opacity: linkingGoogle ? 0.6 : 1 }}
          >
            <SettingsRow
      ...
      {showChangePassword && (
        <div
          style={{ position: 'fixed', inset: 0, zIndex: 80, background: 'rgba(30,26,24,0.45)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '0 32px' }}
          onClick={(e) => { if (e.target === e.currentTarget && passwordStatus.type !== 'loading') closeChangePassword(); }}
        >
          <div style={{ background: 'var(--surface)', borderRadius: 16, width: '100%', maxWidth: 340, overflow: 'hidden' }}>
            {passwordStatus.type === 'success' ? (
              <>
                <div style={{ padding: '28px 24px 20px', textAlign: 'center' }}>
                  <p style={{ fontSize: 16, fontWeight: 600, color: 'var(--text-primary)', margin: '0 0 6px' }}>Password updated</p>
                  <p style={{ fontSize: 14, color: 'var(--text-muted)', margin: 0 }}>Your new password is active.</p>
                </div>
                <div style={{ borderTop: '1px solid var(--border-light)' }}>
                  <button
                    onClick={closeChangePassword}
                    style={{ width: '100%', height: 50, background: 'none', border: 'none', fontSize: 15, color: 'var(--sage)', cursor: 'pointer', fontWeight: 600 }}
                  >Done</button>
                </div>
              </>
            ) : (
              <>
                <div style={{ padding: '24px 20px 16px' }}>
                  <p style={{ fontSize: 16, fontWeight: 600, color: 'var(--text-primary)', margin: '0 0 16px', textAlign: 'center' }}>Change Password</p>
                  {passwordStatus.type === 'error' && (
                    <div style={{ background: 'var(--red-light)', border: '1px solid var(--red-border)', borderRadius: 10, padding: '9px 13px', marginBottom: 14, fontSize: 13, color: 'var(--red)' }}>
                      {passwordStatus.message}
                    </div>
                  )}
                  <div style={{ marginBottom: 12 }}>
                    <label style={{ display: 'block', fontSize: 13, fontWeight: 600, color: 'var(--text-primary)', marginBottom: 5 }}>New password</label>
                    <input
                      type="password"
                      ...
                      autoFocus
                      style={{ width: '100%', padding: '11px 13px', borderRadius: 10, border: '1.5px solid var(--border)', fontSize: 15, color: 'var(--text-primary)', background: 'var(--bg)', outline: 'none', boxSizing: 'border-box' }}
                    />
                  </div>
                  <div>
                    <label style={{ display: 'block', fontSize: 13, fontWeight: 600, color: 'var(--text-primary)', marginBottom: 5 }}>Confirm new password</label>
                    <input
                      type="password"
                      placeholder="••••••••"
                ...
                <div style={{ borderTop: '1px solid var(--border-light)', display: 'flex' }}>
                  <button
                    onClick={closeChangePassword}
                    disabled={passwordStatus.type === 'loading'}
                    style={{ flex: 1, height: 50, background: 'none', border: 'none', borderRight: '1px solid var(--border-light)', fontSize: 15, color: 'var(--text-secondary)', cursor: passwordStatus.type === 'loading' ? 'not-allowed' : 'pointer', fontWeight: 500, opacity: passwordStatus.type === 'loading' ? 0.5 : 1 }}
                  >Cancel</button>
                  <button
                    onClick={handleChangePassword}
                    disabled={passwordStatus.type === 'loading'}
                    style={{ flex: 1, height: 50, background: 'none', border: 'none', fontSize: 15, color: 'var(--sage)', cursor: passwordStatus.type === 'loading' ? 'not-allowed' : 'pointer', fontWeight: 600, opacity: passwordStatus.type === 'loading' ? 0.6 : 1 }}
                  >{passwordStatus.type === 'loading' ? 'Saving…' : 'Save'}</button>
                </div>
              </>
            )}
          </div>

...
export default function PersonFamilyPicker({
  data, initialFamilyIds = [], initialPersonIds = [], onConfirm, onBack,
  ...
  const families = data.families.filter((f) => {
    ...
    return data.people
      ...
      .some((m) => m.englishName.toLowerCase().includes(q) || (m.chineseName && m.chineseName.includes(q)));
  });
      ...
      <div style={{ flex: 1, overflowY: 'auto', minHeight: 0 }}>
        {families.length > 0 && (
          <div style={{ marginBottom: 8 }}>
            <p style={{ fontSize: 10, fontWeight: 600, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 6 }}>Families</p>
            {families.map((f, fi) => {
              const members = data.people.filter((p) => f.memberIds.includes(p.id));
              const palette = avatarPalette[fi % avatarPalette.length];
              const initials = f.label.split(' ').map((w) => w[0]).join('').toUpperCase().slice(0, 2);
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    border: 'none', borderBottomColor: 'var(--border-light)',
        ...
        {individuals.length > 0 && (
          <div>
            <p style={{ fontSize: 10, fontWeight: 600, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 6 }}>Individuals</p>
            {individuals.map((p, pi) => {
              ...
              return (
                ...
                >
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <p style={{ fontSize: 14, fontWeight: 600, color: 'var(--text-primary)', marginBottom: 1 }}>{p.englishName}</p>
                    {p.chineseName && <p style={{ fontSize: 12, color: 'var(--text-muted)' }}>{p.chineseName}</p>}
                  </div>
                  <CheckCircle selected={selected} />
                </button>
              );
        ...
        {families.length === 0 && individuals.length === 0 && (
          <p style={{ fontSize: 13, color: 'var(--text-muted)', fontStyle: 'italic', paddingTop: 16 }}>No results found.</p>
        )}
      </div>
    </div>
  );

...
export default function GroupPreviewModal({ groupId, onClose }: Props) {
  ...
  return (
    <div
      style={{ position: 'fixed', inset: 0, zIndex: 60, background: 'rgba(30,26,24,0.45)', display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }}
    ...
    >
      <div
        ...
        style={{
          ...
          paddingBottom: 'env(safe-area-inset-bottom, 16px)', overflow: 'hidden',
        }}
      >
        {/* Drag handle */}
        ...
        {/* Header */}
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 20px 12px', flexShrink: 0, borderBottom: '1px solid var(--border-light)' }}>
          <button
            onClick={onClose}
          ...
          <span style={{ fontSize: 15, fontWeight: 600, color: 'var(--text-primary)', flex: 1, textAlign: 'center', padding: '0 8px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
            {group.name}
          </span>
          <Link
            href={`/groups/${group.id}`}
            onClick={onClose}
            style={{ fontSize: 13, color: 'var(--sage)', textDecoration: 'none', whiteSpace: 'nowrap', flexShrink: 0 }}
          >
            View full group
          </Link>
        </div>
        ...
        {/* Scrollable content */}
        <div style={{ flex: 1, overflowY: 'auto', padding: '16px 20px 8px', background: 'var(--bg)' }}>
          ...
          {leaders.length > 0 && (
            <div style={{ marginBottom: 14 }}>
              <p style={{ fontSize: 10, fontWeight: 600, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 8 }}>Leaders</p>
          ...
          {members.length > 0 && (
            <div style={{ marginBottom: 8 }}>
              <p style={{ fontSize: 10, fontWeight: 600, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 8 }}>Members · {members.length}</p>
              <div className="no-last-border" style={{ background: 'var(--surface)', borderRadius: 'var(--radius)', border: '1px solid var(--border-light)', overflow: 'hidden' }}>
                {members.map((m, i) => {
                  const palette = avatarPalette[i % avatarPalette.length];
                  const initials = m.englishName.split(' ').map((n) => n[0]).join('').toUpperCase().slice(0, 2);
                  return (
                    <div key={m.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 16px', borderBottom: '1px solid var(--border-light)' }}>
                      <div style={{ width: 36, height: 36, borderRadius: '50%', background: palette.bg, color: palette.color, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 600, flexShrink: 0 }}>
                        {initials}
                      </div>
                      <div>
                        <p style={{ fontSize: 14, fontWeight: 500, color: 'var(--text-primary)', margin: 0 }}>{m.englishName}</p>
                        {m.chineseName && <p style={{ fontSize: 12, color: 'var(--text-muted)', margin: 0 }}>{m.chineseName}</p>}
                      </div>
                    </div>
                  );
                })}
